...
\begin{document}
\begin{frontmatter}
\title{Interférences lumineuses en lumière non polarisée - Division d'amplitude - Interféromètre de Michelson (O3)}
...
$\delta_{1/2} = \delta = (SLIKJM_{\infty})-(SLIHM_{\infty})$. \\
Or, $(SLIKJM_{\infty}) = (SL)+(LI)+(IKJ)+(JM_{\infty})$ et
$(SLIHM_{\infty}) = (SL)+(LI)+(IH)+(HM_{\infty})$. Il vient :
$\delta = (IKJ)+(JM_{\infty}) - (IH)-(HM_{\infty})$. \\
Or, $M_{\infty}$ est le point de concours des rayons interférant à
...
Ainsi, $(\Sigma)$ est un plan d'ondes issu de $M_{\infty}$. Comme
$H$ (projeté orthogonal de J sur le rayon immédiatement réfléchi par
la lame d'air) et $J \in (\Sigma)$, $(M_{\infty}H) = (M_{\infty}J)$
ou encore à l'aide du principe du retour inverse de la lumière : $(HM_{\infty}) = (JM_{\infty})$. \\
On en déduit avec une joie évidente et non dissimulée que : \\
\fbox{$\delta = (IKJ)-(IH)=(IK)+(KJ)-(IH)$}. \\
...
On en déduit que : $\delta = \frac{2e}{\cos i} - 2e\tan i\sin i$ $=$
$\frac{2e}{\cos i}[1-\sin^{2}i] =\frac{2e}{\cos i}[\cos^{2}i] =
2e\cos i$.
\\
...
problème) : $\delta_{1/2} = \delta =
(S_{0}LPQM_{\infty})-(S_{0}LIHM_{\infty})$. \\
On a : $(S_{0}LPQM_{\infty}) = (S_{0}LP)+(PQ)+(QM_{\infty})$.
\\
Or, $(S_{0}LP) = (S_{1}'P)$ car $S_{1}'$ est le symétrique de
$S_{0}$ par le miroir $M_{1}$. Il vient : \\
$(S_{0}LPQM_{\infty}) = (S_{0}LP)+(PQ)+(QM_{\infty}) =
(S_{1}'P)+(PQ)+(QM_{\infty}) = (S_{1}'Q)+(QM_{\infty})$. \\
Enfin, $S_{1}$ étant le symétrique de $S_{1}'$ par la séparatrice,
$(S_{1}'Q) = (S_{1}Q)$. On en déduit que : $(S_{0}LPQM_{\infty}) =
(S_{1}'Q)+(QM_{\infty}) = (S_{1}Q)+(QM_{\infty}) =(S_{1}M_{\infty})$. \\
De même, $(S_{0}LIHM_{\infty})=(S_{0}L)+(LI)+(IHM_{\infty})$. \\
Or, $(S_{0}L) = (S_{2}'L)$ car $S_{2}'$ est le symétrique de
$S_{0}$ par la séparatrice. Il vient : \\
$(S_{0}LIHM_{\infty}) = (S_{0}L)+(LI)+(IHM_{\infty})$ $=$
$(S_{2}'L)+(LI)+(IHM_{\infty})$ $=$ $(S_{2}'I) + (IHM_{\infty})$. \\
Or, $(S_{2}'I) = (S_{2}I)$ car $S_{2}$ est le symétrique de $S_{2}'$
par le miroir $M_{2}$. \\
On en déduit que : $(S_{0}LIHM_{\infty}) = (S_{2}'I) +
(IHM_{\infty})$ $=$ $ (S_{2}I)+(IHM_{\infty})$ $=$
$(S_{2}M_{\infty})$.
...
$\delta = (S_{1}M)-(S_{2}M)$. \\
Or, $(S_{1}M) = (S_{1}H)+(HM_{\infty})$. $M_{\infty}$ est le point
...
$S_{2} \in (\Sigma)$, $(M_{\infty}H) = (M_{\infty}S_{2})$
ou encore à l'aide du principe du retour inverse de la lumière : $(HM_{\infty}) = (S_{2}M_{\infty})$. \\
Il vient : $\delta = (S_{1}H)$. Dans l'air, $\delta = S_{1}H$. Or,
...
élémentaire $dI_{0}$. A partir d'un incident (à $i$ fixé) issu d'un
quelconque point source $S \in [AB]$, la différence de marche entre
deux émergents parallèlement en sortie de l'inter-féromètre vaut
invariablement $\delta = (S_{1}M_{\infty})-(S_{2}M_{\infty}) =
2e\cos i$.
...
En notant $dI_{0}(S_{1}) =dI_{0}(S_{2})$ l'intensité d'une des
sources secondaires, on a : $dI(M_{\infty}) =
2dI_{0}(S_{1})[1+\cos(\Delta \varphi)]$ avec $\forall S_{1}\in
[S_{A1}S_{B1}]$ et $S_{2}  \in [S_{A2}S_{B2}]$, $\Delta \varphi =
\frac{2\pi}{\lambda_{0}}\delta$.
\item Les différentes sources
ponctuelles $S$ formant la source étendue sont incohérentes entre
elles : les intensités qu'elles produisent en $M_{\infty}$
s'ajoutent. \\
L'intensité produite en $M_{\infty}$ de la part des sources
secondaires étendues est ainsi : \\
$I(M_{\infty}) = \int_{S_{1} \in [S_{A1}S_{B1}]}
2dI_{0}(S_{1})[1+\cos(\Delta \varphi)]$, ou encore : $I(M_{\infty})
= [\int_{S_{1} \in [S_{A1}S_{B1}]} 2dI_{0}(S_{1})][1+\cos(\Delta
\varphi)]$, d'où : \fbox{$I(M_{\infty}) = 2I_{0}[1+\cos(\Delta
\varphi)]$}. \\
...
: $\delta = (S_{0}LIKJBM)-(S_{0}LIHAM)$. \\
Or, $(S_{0}LIKJBM) = (S_{0}L)+(LI)+(IKJBM)$ et $(S_{0}LIHAM) =
(S_{0}L)+(LI)+(IHAM)$, d'où : $\delta = (IKJBM) - (IHAM)$.
\\
Or, $(IKJBM) = (IKJ)+(JBM)$ et $(IHAM) = (IH)+(HAM)$. Or, le point
...
montrâmes que $(\Sigma)$ est un plan d'ondes issu de $M$ ; il vient
que : $(HAM) = (JBM)$. \\
...
2I_{0}[1+\cos(\frac{2\pi}{\lambda_{0}}\times 2e\cos i)]$. \\
...
centre $F'$.
\item Ecrivons que $\delta = 2e\cos i \approx 2e[1 -
\frac{i^{2}}{2}]$ (DL à l'ordre 2 sur $\cos i$ car $i << 1$). \\
Par ailleurs (Cf. Annexe n$^{\circ}$3, figure de droite), $\tan i
\approx i = \frac{r}{f'}$, d'où : $\delta \approx 2e[1-
\frac{r^{2}}{2f'^{2}}]=p\lambda_{0}$ avec $p$ l'ordre
d'interférences en $M$. Il vient : $p = \frac{2e}{\lambda_{0}}[1-\frac{r^{2}}{2f'^{2}}]$. \\
Notons $p_{0}$ l'ordre au centre (acquis pour $r = 0$) ; on a :
$p_{0} = \frac{2e}{\lambda_{0}}$. On en déduit que : \fbox{$p =
p_{0}[1-\frac{r^{2}}{2f'^{2}}]$}. \\
...
d'interférences : $p = p_{0}[1-\frac{r_{p}^{2}}{2f'^{2}}]$. Il vient
: $1 - \frac{p}{p_{0}} = \frac{r_{p}^{2}}{2f'^{2}}$ ; on en déduit
que : \fbox{$r_{p} = f'\sqrt{2[1-\frac{p}{p_{0}}]}$}.
...
existe un premier entier naturel $p_{1} = E(p_{0}) = \llcorner p_{0}
...
f'\sqrt{2[1-\frac{p_{k}}{p_{0}}]}$ \\
$=$ $f'\sqrt{2[1-\frac{p_{1}-[k-1]}{p_{0}}]}$ $=$
$f'\sqrt{\frac{2}{p_{0}}[p_{0}-p_{1}+[k-1])]}$. \\
...
\fbox{$r_{k} = f'\sqrt{\frac{\lambda_{0}}{e}[\epsilon +k-1]}$}.
\end{itemize}
...
\noindent Sur la figure de gauche, $\cos(2\alpha) =
\frac{e(x)}{PK}$. Or, $\alpha << 1$, donc à l'ordre le plus bas,
$\cos(2\alpha) \approx 1$. A cet ordre d'approximation (DL à l'ordre
...
(S_{0}CLIPIM_{\infty})-(S_{0}CLILM_{\infty})$. Or,
$(S_{0}CLILM_{\infty}) = (S_{0}C) +(CL)+(LI)+(ILM_{\infty})$ et
$(S_{0}CLIPIM_{\infty}) =
(S_{0}C)+(CL)+(LI)+(IP)+(PI)+(ILM_{\infty})$. D'où (on travaille
dans l'air) : $\delta = (IP)+(PI) = 2e(x)$. \\
Or, $\tan(\alpha) \approx \alpha = \frac{e(x)}{x}$, d'où : $e(x)
\approx \alpha x$. On en déduit la différence de marche :
\fbox{$\delta = 2\alpha x$}.
\\
...
\begin{itemize}
\item On calcule $\delta_{1/2} =
(S_{0}CLIPKQM)_{rayon\,1}$\\ $-(S_{0}CLILRM)_{rayon\,2}$.
\item Or,
$(S_{0}CLIPKQM)_{rayon\,1} =
(S_{0}C)+(CL)+(LI)+(IP)+(PKQM)_{rayon\,1}$ et
$(S_{0}CLILRM)_{rayon\,2} = (S_{0}C)+(CL)+(LI)+(ILRM)_{rayon\,2}$.
\\ Il reste à calculer : $\delta = \delta_{1/2} =
(IP)+(PKQM)_{rayon\,1} - (ILRM)_{rayon\,2}$.
\item Exploitons le fait que $M$ est le conjugué de $P$ par la
seconde lentille mince convergente : $P
\underleftrightarrow{(LMC2)}M$, soit : $(PKQM)_{rayon\,1} =
(PILRM)_{rayon\,2}$. \\
Or, $(PILRM)_{rayon\,2} = (PI)+(ILRM)_{rayon\,2}$. On en déduit que
: $\delta = \delta_{1/2} = (IP)+(PILRM)_{rayon\,2} -
(ILRM)_{rayon\,2}$ $=$ $(IP)+(PI) = 2IP = 2e(x) \approx 2\alpha x$.
\end{itemize}

\subsection*{3. Calcul de l'intensité}
\noindent Soit $I_{0}$ l'intensité du point source $S_{0} \in [AB]$.
Le point de concours des rayons interférant est un point $P \in
(M_{1}')$. Or, les rayons émergents de l'interféromètre ont subi une
...
\frac{I_{0}}{4}[1+\cos(\Delta \varphi(P))]$ avec $\Delta \varphi(P)
= \frac{2\pi}{\lambda_{0}}\times 2\alpha x$, soit : \\
$I(P) = I(x) = \frac{I_{0}}{2}[1+\cos(\frac{2\pi}{\lambda_{0}}\times
2\alpha x)]$.

\subsection*{4. Nature des franges}
\begin{itemize}
\item Les franges d'égale intensité sont les franges d'égale $x$ : les
franges sont donc rectilignes (d'équation $x = cste$), parallèles à
...
p_{2}[\lambda_{0}+\frac{\Delta \lambda}{2}]$. \\
On en déduit en $M$
...
>  \ell_{c} = \frac{\lambda_{0}[\lambda_{0}+\frac{\Delta \lambda}{2}]}{\Delta \lambda}$. \\
...
nulle} \noindent Pour tout $\lambda \in [\lambda_{1},\,
\lambda_{2}]$, au centre de la figure, pour $e = 0$, $p_{0} =
...
"faible"} \noindent Au centre de la figure, $\delta = 2e$. Il existe
des longueurs d'onde $\lambda \in [\lambda_{1},\, \lambda_{2}]$
éteintes. En effet, l'ordre au centre varie entre les valeurs
$p_{min} = \frac{2e}{\lambda_{2}} = \frac{1125}{800} = 1,4$ et
$p_{max} = \frac{2e}{\lambda_{1}} = \frac{1125}{400} = 2,8$. \\
