...
\begin{document}
\begin{frontmatter}
\title{Cours EM5 bis : Induction électromagnétique}
...
\noindent L'équation de Maxwell permettant d'expliquer le phéno-mène
d'induction est l'équation de Maxwell-Faraday (M.F.): \\
\fbox{$\overrightarrow{rot}\overrightarrow{E}= -\frac{\partial
...
\overrightarrow{A}}{\partial t} = -\overrightarrow{grad}V$, ou
encore (en rajoutant les fioritures
d'usage) : \\
\fbox{$\overrightarrow{E}(M,t) = -\frac{\partial
...
\noindent Soit un circuit filiforme, fixe dont le contour $(\Gamma)$
a
...
\end{eqnarray}
Or, $\phi(t) = \iint_{P\in
S}\overrightarrow{B}(P,t).\overrightarrow{dS}(P)$ est le flux du
...
\fbox{$e(t) = -\frac{d\phi}{dt}(t)$}.
\subsection*{5. Induction de Neumann dans un circuit filiforme}
\textbf{b)  Exemple d'application : spire circulaire plongée dans un
...
choisi, $\overrightarrow{dS}=dS\overrightarrow{u}_{z}$ ; le champ
magnétique est uniforme MAIS dépendant du temps): $\phi(t) =
\iint_{P\in S} \overrightarrow{B}.\overrightarrow{dS}(P) = B_{0}\cos
\omega t \pi r^{2}$. \\
Conformément à la loi de Faraday, $e(t) = -\frac{d\Phi}{dt} =
B_{0}\omega \sin\omega t \pi r^{2}$, puis $i(t) = \frac{e}{R} =
\frac{B_{0}\omega \sin\omega t \pi r^{2}}{R}$.
\end{enumerate}

\subsection*{6. Auto-induction}
\textbf{c)  Coefficients de Neumann} \\
\noindent Considérons deux circuits filiformes $\mathcal{C}_{1}$ et
...
\fbox{$\Phi_{1\rightarrow 1}(t) = L_{1}i_{1}(t)$} et
...
\fbox{$\Phi_{1\rightarrow 2}(t) = M_{12}i_{1}(t)$} et
...
\textbf{d) Exercice d'application} \\
\noindent \underline{Prérequis :} le flux est additif : si la
...
$\overrightarrow{dS} =dS\overrightarrow{u}_{z}$. \\
...
$\overrightarrow{dS}_{2} =dS_{2}\overrightarrow{u}_{z2}$. \\
...
spire\, de\,2} = \mu_{0}\frac{NI}{\ell}S_{2}\cos\theta$. Or,
$\Phi_{1\longrightarrow 2}(t) = N_{2}\Phi_{1\longrightarrow 1
spire\, de\,2}$, donc le flux du champ magnétique créé par le grand
...
relative. \\
En effet, si $\theta \in [0,\frac{\pi}{2}] \cup
[\frac{3\pi}{2},2\pi]$, $M\geq 0$ ($\cos\theta \geq 0$) ; si $\theta
\in [\frac{\pi}{2},\frac{3\pi}{2}]$, $M\leq 0$ ($\cos\theta \leq
0$).
\end{itemize}

\textbf{f)  Utilisation des coefficients de Neumann} \\
...
\textbf{b)  Energie magnétique de deux circuits couplés} \\
\noindent Les variations temporelles du flux du champ magnétique
...
-L_{1}\frac{di_{1}}{dt}-M\frac{di_{2}}{dt}$ et $e_{2} =
-L_{2}\frac{di_{2}}{dt}-M\frac{di_{1}}{dt}$. \\
...
-e_{1}+r_{1}i_{1} =
L_{1}\frac{di_{1}}{dt}+M\frac{di_{2}}{dt}+r_{1}i_{1}$ et $E_{2} =
-e_{2}+r_{2}i_{2} =
L_{2}\frac{di_{2}}{dt}+M\frac{di_{1}}{dt}+r_{2}i_{2}$. Faisons la
somme de $E_{1}i_{1}+E_{2}i_{2}$ ; on a : \\
$E_{1}i_{1}+E_{2}i_{2} =
L_{1}i_{1}\frac{di_{1}}{dt}+Mi_{1}\frac{di_{2}}{dt}+r_{1}i_{1}^{2}+
L_{2}i_{2}\frac{di_{2}}{dt}+Mi_{2}\frac{di_{1}}{dt}+r_{2}i_{2}^{2}$,
ou encore : \\
$E_{1}i_{1}+E_{2}i_{2} =
\frac{d}{dt}[\frac{1}{2}L_{1}i_{1}^{2}+\frac{1}{2}L_{2}i_{2}^{2}
+Mi_{1}i_{2}]+ r_{1}i_{1}^{2}+r_{2}i_{2}^{2}$. \\
On pose (à une constante inessentielle près), l'énergie potentielle
des deux circuits couplés : \\
\fbox{$E_{p} = \frac{1}{2}L_{1}i_{1}^{2}+\frac{1}{2}L_{2}i_{2}^{2} +
Mi_{1}i_{2}$}.
\\
Au final, $E_{1}i_{1}+E_{2}i_{2} = \frac{dE_{p}}{dt}+r_{1}i_{1}^{2}
+r_{2}i_{2}^{2}$. \\
...
Espace}w_{m}(M,t)dV(M)$ avec $w_{m} = \frac{B^{2}}{2\mu_{0}}$, donc
$E_{p} = \iiint_{M \in (\mathcal{D})} \frac{B^{2}}{2\mu_{0}}dV(M)
...
\frac{1}{2}L_{1}i_{1}^{2}+[Mi_{2}]i_{1}+ \frac{1}{2}L_{2}i_{2}^{2}$.
\\
...
(\mathcal{D}_{1})}\frac{B_{1}^{2}}{2\mu_{0}}(M,t)dV(M)=\frac{1}{2}L_{1}i_{1}^{2}(t)$.

\section*{II. Induction de Lorentz}
\subsection*{2. Changement de référentiel galiléen en électromagnétisme}
\textbf{a) Position du problème : " un couac " de la mécanique
newtonienne} \\
\noindent Soit (R) le référentiel du laboratoire galiléen et (R') le
...
\textbf{c)  Vérification dans la limite de la transformation
galiléenne (mécanique newtonienne) pour l'ARQS magnétique}
\\
\noindent La démonstration (magnifique !) se fait dans le cadre de
...
\fbox{$\overrightarrow{E'}=\overrightarrow{E}+\overrightarrow{v}_{e}
\times \overrightarrow{B}$}.

\subsection*{3. Champ électromoteur de Lorentz et force électromotrice
de Lorentz} \textbf{ a)  Définitions :} \\
\noindent On définit (au sens de Lorentz) le champ électromoteur par
...
\noindent La loi de Faraday demeure licite dans le cadre de
...
\noindent Soit un circuit filiforme parcouru par un courant
...
expression : \\
\fbox{$\overrightarrow{F}_{L} = \oint_{M \in
...
générateurs)} \noindent A $t = 0$, un opérateur communique une
...
\Gamma}i(t)\overrightarrow{d\ell}(M)\times \overrightarrow{B} =
i\int_{A}^{C}[dy\overrightarrow{u}_{y}] \times
[B\overrightarrow{u}_{z}] = i\ell B \overrightarrow{u}_{x}$. \\
...
$e(t) = \oint_{M \Gamma}
\overrightarrow{E}_{m}(t).\overrightarrow{d\ell}(M)=
\int_{A}^{C}[-vB\overrightarrow{u}_{y}].[dy\overrightarrow{u}_{y}]$
. \\
Il vient : \fbox{$e(t)=-v(t)B\ell$}. \\
Tentons maintenant de calculer la fem induite\footnote{"Passage en
force" !} par la loi de Faraday : entre $t$ et $t+dt$ la surface du
circuit a varié de $dS(t) = \ell dx(t) = \ell v(t)dt$. Le flux a
varié de ($\overrightarrow{dS}$ est dirigé comme
$\overrightarrow{B}$) $d\Phi =
\overrightarrow{B}.\overrightarrow{dS} = vB\ell dt$. Il vient :
\fbox{$e(t) =
-v(t)B\ell$}. \\
...
\fbox{$\overrightarrow{F} =i\ell B\overrightarrow{u}_{x} =
...
\textbf{10) Principe des moteurs électriques} \\
\textbf{a)  Moteurs linéaires (machines à courant continu) à partir des rails de Laplace} \\
On garde les mêmes
...
$m\frac{dv}{dt} = i\ell B = \frac{B \ell E}{R} -
\frac{B^{2}\ell^{2}}{R}v$. \\
On divise par $m$, on introduit le
temps caractéristique $\tau = \frac{mR}{B^{2}\ell^{2}}$ et on
aboutit à
l'équation : $\frac{dv}{dt}+\frac{v}{\tau} = \frac{B\ell}{mR}E$. \\
...
induit d'intensité : $i_{induit} =
\frac{e}{R} = -\frac{vB\ell}{R}$. \\
D'après la loi de modération de Lenz, $i_{induit}$ tend à s'opposer
aux causes qui lui ont donné naissance : $d\Phi = B\ell v dt >0$,
donc $e(t) < 0$, donc $i_{induit} < 0$ : $i_{induit}$ s'oppose ici à
$I_{0}$. Ceci perdure jusqu'à une vitesse acquise $v_{lim}$ par le
barreau pour laquelle $i_{induit}$ compense $I_{0}$ : on a alors
$I_{0}-\frac{v_{lim}B\ell}{R}=0$, soit : $v_{lim} = \frac{RI_{0}}{B
\ell} = \frac{E}{B \ell}$.\\

\textbf{b)  Machines rotatives à courant continu} \\
...
=\overrightarrow{F}.\overrightarrow{d\ell}$ ; pour des actions qui
...
\overrightarrow{\Gamma}.\overrightarrow{\omega}dt$.}
$\mathcal{P}_{Laplace} =
...
\textbf{1. Position du problème et expression de la f.e.m. induite
dans le cas général} \\
...
S}\overrightarrow{rot}\overrightarrow{E}.\overrightarrow{dS}(P)+\oint_{M\in
\Gamma}[\overrightarrow{v}_{e}\times
\overrightarrow{B}].\overrightarrow{d\ell}(M)$. \\
...
\textbf{2.  Cas limites des inductions de Neumann et Lorentz comme
cas particuliers}
\begin{itemize}
...
t}(P,t).\overrightarrow{dS}(P)= -\frac{d}{dt}\Phi(t)$ avec $\Phi(t)
= \iint_{P\in S}\overrightarrow{B}(P,t).\overrightarrow{dS}(P)$. La
...
\textbf{3. Loi de Faraday dans les circuits de constitution rigide en translation rectiligne uniforme par rapport (R)} \\
...
-\frac{d}{dt}\iint_{P\in
S}\overrightarrow{B}(P,t).\overrightarrow{dS}(P)$ $=$
$-\frac{d\Phi}{dt}(t)$ : la loi de Faraday demeure licite.

\textbf{4.  Exit la loi de Faraday dans les circuits de constitution
variable ...} \\
